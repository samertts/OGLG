...
class TestArchiveTransitions:
    def test_active_to_archived(self) -> None:
        validate_archive_transition(ArchiveStatus.ACTIVE, ArchiveStatus.ARCHIVED)

    def test_active_to_soft_deleted(self) -> None:
        validate_archive_transition(ArchiveStatus.ACTIVE, ArchiveStatus.SOFT_DELETED)

    def test_soft_deleted_to_active(self) -> None:
        validate_archive_transition(ArchiveStatus.SOFT_DELETED, ArchiveStatus.ACTIVE)

    def test_archived_to_active(self) -> None:
        validate_archive_transition(ArchiveStatus.ARCHIVED, ArchiveStatus.ACTIVE)

    def test_purged_has_no_transitions(self) -> None:
        with pytest.raises(StateTransitionError):
            validate_archive_transition(ArchiveStatus.PURGED, ArchiveStatus.ACTIVE)

    def test_active_to_purged_fails(self) -> None:
        with pytest.raises(StateTransitionError):
            validate_archive_transition(ArchiveStatus.ACTIVE, ArchiveStatus.PURGED)
